...
func (t *ApplyPatchTool) Descriptor() Descriptor {
	return Descriptor{
		ID:           t.ID(),
		Description:  "Apply reviewed workspace edits written as aider SEARCH/REPLACE blocks: a file path, <<<<<<< SEARCH, existing lines, =======, replacement lines, and >>>>>>> REPLACE. Explicit path lines may repeat for multiple edits to the same file, but their SEARCH ranges must not overlap. Empty SEARCH creates a missing file or matches an empty file. SEARCH matches regardless of LF, CRLF, or CR terminators; replacements use the matched location's terminator, while untouched bytes (including mixed line endings and a UTF-8 BOM) are preserved. Set format to \"unified\" for git-style unified diff text with the same line-ending behavior.",
		Schema:       t.JSONSchema(),
...
func (*ApplyPatchTool) JSONSchema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{"patchText":{"type":"string","description":"The patch text, written in the format named by the format field. For updates, patch and target line endings need not match: SEARCH/context lines match LF, CRLF, or CR, replacement lines use the target file's matched-location line ending, and untouched bytes preserve exact terminators and any UTF-8 BOM."},"format":{"type":"string","enum":["aider","unified"],"description":"Edit syntax of patchText, defaulting to aider. \"aider\": one or more SEARCH/REPLACE blocks, each a workspace-relative or explicitly security-authorized absolute file path on its own line, then <<<<<<< SEARCH, the exact lines to replace, =======, the replacement lines, and >>>>>>> REPLACE; explicit path lines may repeat for several edits to one file, but the same file's SEARCH ranges must not overlap; leave the SEARCH section empty to create a missing file or match an existing empty file. \"unified\": git diff text with --- and +++ headers and @@ hunks; a /dev/null source creates the file and a /dev/null target deletes it, and renames are rejected. Both update formats are line-ending agnostic and preserve untouched bytes exactly."}},"required":["patchText"],"additionalProperties":false}`)
}
