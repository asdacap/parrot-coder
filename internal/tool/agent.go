...
func AgentDescriptor(kind string) Descriptor {
	...
	case agentSpawnID:
		...
		descriptor.SystemPromptGuidance = "Do not duplicate work with agent_spawn. If an agent is already handling the work and is still running, wait for it to finish instead of spawning another agent for the same work."
		...
		descriptor.Presentation = Presentation{Subagent: true, SuccessIcon: "♟", Label: LabelSpec{Fields: []LabelField{{Names: []string{"name", "agent"}}}}, CompletedInput: CompletedInputSpec{Fields: []string{"name", "agent", "model", "prompt"}, TerminalOnly: true}}
	case agentSendID:
