...
`exec_command` runs in a mandatory operating-system sandbox by default:
...
worktree common Git directories outside the workspace are also writable. Process
execution fails when the sandbox is unavailable. Setting `sandbox_permissions`
to `disable_sandbox` requires permission and runs with the invoking user's local
authority. Configured local
